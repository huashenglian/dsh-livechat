...
import {
  ...
  GIFT_EXT,
  uid,
  kindForExt as giftKindForExt,
  addAsset as addGiftAsset,
  createFolder as createGiftFolder,
  renameFolder as renameGiftFolder,
  deleteFolder as deleteGiftFolder,
  moveItems as moveGiftItems,
  deleteItems as deleteGiftItems,
  setWeight as setGiftWeight,
  clearAll as clearGiftAll,
  toPublicItem as toPublicGiftItem,
  toPublicFolder as toPublicGiftFolder,
  sanitizeSvg,
  ensureSeed as ensureGiftSeed,
} from './gift-lib.js'
...
// ---- gift catalog route (todo 5) -------------------------------------------
// Read path: prefer the on-disk `$DSH_HOME/danmaku-gifts/catalog.json`; when it
// is absent, seed it from the catalog committed with the package
// (`assets/gift-catalog.json`) and copy that to disk. A tiny memory cache keyed
// on mtime+size avoids re-parsing ~700KB on every poll; any refresh invalidates
// it. The refresh path takes the room id from `config.giftRoomId` (default '')
// — the id is never logged, echoed or written anywhere.
const GIFT_LIVE_API = 'https://api.live.bilibili.com'
const GIFT_CATALOG_UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
let giftCatalogCache = null

/** The catalog shipped inside this bundle. */
function giftBuiltinCatalogPath() {
  return join(dirname(fileURLToPath(import.meta.url)), '..', 'assets', 'gift-catalog.json')
}

function readGiftCatalogJson(p) {
  try {
    const j = JSON.parse(readFileSync(p, 'utf8'))
    if (j && Array.isArray(j.gifts)) return j
  } catch { /* missing / malformed */ }
  return null
}

/** Atomic catalog write (tmp + rename): a failure never corrupts an existing
 *  catalog.json, and leaves no temp file behind. */
function writeGiftCatalogAtomic(payload, home) {
  try {
    mkdirSync(giftDir(home), { recursive: true })
    const p = giftCatalogPath(home)
...
/** Effective catalog: disk first (mtime-cached), else seed from the package
 *  copy and persist it. Returns null only when neither copy exists. */
function readGiftCatalog(home) {
  const p = giftCatalogPath(home)
  let st = null
  try { st = statSync(p) } catch { st = null }
  if (st && giftCatalogCache && giftCatalogCache.path === p
    && giftCatalogCache.mtimeMs === st.mtimeMs && giftCatalogCache.size === st.size) {
    return giftCatalogCache.payload
  }
  const disk = st ? readGiftCatalogJson(p) : null
  if (disk) {
    giftCatalogCache = { path: p, mtimeMs: st.mtimeMs, size: st.size, payload: disk }
    return disk
  }
  const builtin = readGiftCatalogJson(giftBuiltinCatalogPath())
  if (!builtin) return null
  const wrote = writeGiftCatalogAtomic(builtin, home)
  if (wrote) {
    try {
      const st2 = statSync(p)
      giftCatalogCache = { path: p, mtimeMs: st2.mtimeMs, size: st2.size, payload: builtin }
    } catch { /* keep serving from memory */ }
  }
  return builtin
}

/** Host-side `giftConfig(pc)` fetch — async with a 20s cap so the event loop is
 *  never blocked. Returns the raw gift array or throws (caller degrades). */
async function fetchLiveGiftCatalog(roomId) {
  const url = GIFT_LIVE_API + '/xlive/web-room/v1/giftPanel/giftConfig?platform=pc&room_id=' + encodeURIComponent(roomId)
  const r = await fetch(url, {
    headers: {
      'User-Agent': GIFT_CATALOG_UA,
  ...
  if (j && j.code != null && j.code !== 0) throw new Error('api code ' + j.code)
  const d = j && j.data
  const cands = [
    d && d.gift_config && d.gift_config.base_config && d.gift_config.base_config.list,
    d && d.list,
    d && d.data && d.data.list,
    Array.isArray(d) ? d : null,
  ]
  for (const c of cands) if (Array.isArray(c) && c.length) return c
  throw new Error('no gift list')
}

// ---- gift upload / import channels (todo 3) --------------------------------
// The emoji upload path tunnels binaries through base64 JSON (readBody has no
// cap). Gift assets use a raw binary body with a hard byte cap read from
// `config.giftMaxAssetMB` — nothing here is unbounded and the cap is never
// hard-coded.

/** Read a request body with a hard byte cap. Buffers at most cap + one chunk. */
function readCappedBody(req, maxBytes) {
  return new Promise((resolve) => {
    ...
    req.on('data', (c) => {
      total += c.length
      if (total > maxBytes) { over = total; return } // keep draining, buffer nothing
      chunks.push(c)
    })
    req.on('end', () => {
      if (over) resolve({ ok: false, reason: 'too_large', bytes: over })
      else resolve({ ok: true, buffer: Buffer.concat(chunks), bytes: total })
    })
    req.on('error', () => resolve({ ok: false, reason: 'aborted' }))
  })
}

/** Collapse a client filename to a safe display name — never a path. */
function sanitizeAssetName(raw) {
  let n = String(raw == null ? '' : raw).replace(/[\u0000-\u001f\u007f]/g, '')
  n = n.split(/[\\/]+/).filter((p) => p && p !== '.' && p !== '..').pop() || ''
  n = n.replace(/[<>:"|?*]/g, '_').replace(/^\.+/, '').trim()
  if (n.length > 80) n = n.slice(0, 60) + extname(n).slice(0, 20)
  return n
}

// Fallback extension per upload content-type (an explicit `?name=` ext wins).
const UPLOAD_CT_EXT = {
  'image/svg+xml': '.svg',
  'image/png': '.png',
  'image/jpeg': '.jpg',
...
/** Rebuild a folder chain in a manifest object, reusing existing folders. */
function ensureGiftFolderChain(manifest, parentFolderId, parts) {
...
/** Every whitelisted asset under `root`, recursively. */
function walkGiftFiles(root) {
  const out = []
  const walk = (dir) => {
    let entries
    ...
    for (const ent of entries) {
      ...
      else if (GIFT_EXT.has(extname(ent.name).toLowerCase())) out.push(p)
    }
  }
  walk(root)
  return out
...
// ---- gift import job (single instance; mirrors emoji-lib's job shape) -------
let giftJob = null
let giftAbortFlag = false

function abortGiftImport() {
  giftAbortFlag = true
  if (giftJob) giftJob.aborted = true
}

function getGiftImportStatus() {
  if (!giftJob) return { running: false }
  return {
    running: !!giftJob.running,
    id: giftJob.id,
    type: giftJob.type,
    done: giftJob.done,
    total: giftJob.total,
    current: giftJob.current,
    error: giftJob.error || null,
    aborted: !!giftJob.aborted,
    limit: giftJob.limit,
    folderId: giftJob.folderId || null,
  }
}

function newGiftJob(type, folderId, limit) {
  return {
    id: uid('j_'),
    type,
    running: true,
...
/** PowerShell Expand-Archive (same spawn form as emoji-lib). */
function expandArchive(zipPath, outDir) {
  return new Promise((resolve, reject) => {
    ...
    const p = spawn('powershell', ['-NoProfile', '-Command', cmd], { windowsHide: true })
    let err = ''
    p.stderr.on('data', (d) => { err += String(d) })
    p.on('error', reject)
...
/**
 * Enumerate zip entry names without extracting. PS 5.1 needs the explicit
 * Add-Type — a bare `[IO.Compression.ZipFile]` throws "type not found".
 */
function listZipEntries(zipPath) {
  return new Promise((resolve, reject) => {
    ...
    p.on('close', (code) => (code === 0
      ...
      : reject(new Error(err || ('listzip exit ' + code)))))
  })
}

/** True when a zip entry could escape the extraction root. */
function zipEntryUnsafe(name) {
  const n = String(name || '').replace(/\\/g, '/')
  if (!n) return true
  if (n.startsWith('/')) return true            // absolute (UNC becomes // too)
  if (/^[A-Za-z]:/.test(n)) return true          // drive letter
  return n.split('/').some((p) => p === '..')
}

/** Import whitelisted files from `root`, 4 at a time (mirrors emoji-lib). */
async function copyGiftFiles({ root, job, parentFolderId, limit }) {
  const files = walkGiftFiles(root)
  job.total = files.length
  const concurrency = 4
  let idx = 0
  const runOne = async (filePath) => {
    if (giftAbortFlag || job.aborted) return
    if (limit > 0 && job.done >= limit) return
    const parts = relative(root, filePath).split(/[\\/]/).filter(Boolean)
    const name = parts[parts.length - 1]
    const dirs = parts.slice(0, -1)
    job.current = parts.join('/')
    try {
      const buffer = readFileSync(filePath)
      const ext = extname(filePath).toLowerCase()
      const manifest = loadGiftManifest()
      let folderId = parentFolderId || null
      if (dirs.length) {
        folderId = ensureGiftFolderChain(manifest, folderId, dirs)
        saveGiftManifest(manifest)
      }
      let item
      if (giftKindForExt(ext) === 'svg') {
        const clean = sanitizeSvg(buffer.toString('utf8'))
        item = clean.ok
          ? addGiftAsset({ name, buffer: Buffer.from(clean.svg, 'utf8'), folderId, weight: 1, ext: '.svg' })
          : null
      } else {
        item = addGiftAsset({ name, buffer, folderId, weight: 1, ext })
      }
      if (item) job.done++
    } catch { /* skip unreadable */ }
  }
  const worker = async () => {
    while (idx < files.length && !giftAbortFlag && !job.aborted) {
...
/** Clone a git repo (github / git@ / local file://) then import its assets. */
async function startGiftGitImport({ url, folderId, limit }) {
  if (giftJob && giftJob.running) return { ok: false, reason: 'busy' }
  giftAbortFlag = false
  const job = newGiftJob('git', folderId, limit)
  job.current = 'cloning…'
  giftJob = job
  const repoUrl = String(url || '').trim()
  const tmp = join(tmpdir(), 'dsh-gift-git-' + job.id)
  try {
    if (!/^(https?:\/\/|git@|file:\/\/)/i.test(repoUrl)) throw new Error('invalid git url')
    mkdirSync(tmp, { recursive: true })
    await runGit('git', ['clone', '--depth', '1', repoUrl, tmp], tmpdir())
    if (!giftAbortFlag && !job.aborted) {
      await copyGiftFiles({ root: tmp, job, parentFolderId: folderId, limit: job.limit })
    }
  } catch (e) {
  ...
  return { ok: !job.error, job: getGiftImportStatus() }
}

/**
 * Unzip a buffer: validate every entry name BEFORE extraction (a `..` /
 * absolute / drive entry rejects the whole package), then Expand-Archive and
 * import only relative, whitelisted files.
 */
async function startGiftZipImport({ zipPath, folderId, limit }) {
  if (giftJob && giftJob.running) return { ok: false, reason: 'busy' }
  giftAbortFlag = false
  const job = newGiftJob('zip', folderId, limit)
  job.current = 'validating…'
  giftJob = job
  const outDir = join(tmpdir(), 'dsh-gift-zip-' + job.id)
  try {
    const entries = await listZipEntries(zipPath)
    const bad = entries.find((n) => zipEntryUnsafe(n))
    if (bad) throw new Error('unsafe zip entry: ' + bad)
    mkdirSync(outDir, { recursive: true })
    job.current = 'extracting…'
    await expandArchive(zipPath, outDir)
    // A single top-level folder is treated as the content root.
    let root = outDir
    try {
    ...
    job.current = ''
    await copyGiftFiles({ root, job, parentFolderId: folderId, limit: job.limit })
  } catch (e) {
    job.error = String(e && e.message ? e.message : e)
  } finally {
    job.running = false
...
async function startGiftZipBufferImport({ buffer, folderId, limit }) {
  const zipPath = join(tmpdir(), 'dsh-gift-upload-' + uid('z_') + '.zip')
  writeFileSync(zipPath, buffer)
  return startGiftZipImport({ zipPath, folderId, limit })
}
...
function apply(ctx, _config) {
  ...
  //
  // POLICY — gift assets are USER PROPERTY and are deliberately NOT cleaned up
  // here. Contrast with the emoji cache above: emojis are re-derivable plugin
  // data (a shipped preset library the user can rebuild), so uninstall may drop
  // them. Gift assets under `$DSH_HOME/danmaku-gifts/` are imported/uploaded by
  // the user (own SVGA/GIF/SVG files, folder organisation, weights) and are not
  // reproducible from the package — auto-deleting them on uninstall would
  // destroy user data. They are removed only by the user's explicit
  // 「清空素材」 action (POST /api/danmaku/gift/assets {action:'clearAll'}) or by
  // deleting the folder by hand. Keep this asymmetry: do NOT add clearGiftAll()
  // to this dispose handler.
  ctx.on('dispose', () => {
    try {
      if (!pluginPackageStillInstalled()) {
        ...
        clearEmojiCache()
        clearPresetStore()
        console.log('[dsh-livechat] uninstall cleanup, removed', r.removed, 'pool files + emoji/preset cache (gift assets kept — user property)')
      }
    } catch {
      /* ignore */
    }
  ...
  // First-boot gift asset seed (todo 25). `ensureSeed` copies the packaged
  // `assets/gift-defaults/*.svg` into `$DSH_HOME/danmaku-gifts/` and is a no-op
  // once `manifest.json` exists — so a library the user cleared (manifest kept,
  // files gone) never resurrects, and the seed runs at most once per install.
  // Items land as ordinary store assets tagged `source: 'builtin'`. Guarded so a
  // bad/unreadable package asset can never block boot.
  try {
    const seeded = ensureGiftSeed({ home: process.env.DSH_HOME || join(homedir(), '.dsh') })
    if (seeded && seeded.seeded) console.log('[dsh-livechat] gift defaults seeded:', seeded.seeded)
  } catch {
    /* never block boot on a seed failure */
  }
  ...
  if (webServer) {
      ...
      handler: async (req, res) => {
        try {
          ...
          if (req.method === 'GET') {
            ...
            sendJson(res, 200, {
              ok: true,
              folders: m.folders.map(toPublicGiftFolder),
              items: m.items.map(toPublicGiftItem),
            })
            return
          }
          if (req.method === 'PATCH' || req.method === 'POST') {
            ...
            const action = body && body.action
            if (action === 'folderCreate') {
              sendJson(res, 200, { ok: true, folder: createGiftFolder(body.name, body.parentId, home) })
              return
            }
            if (action === 'folderRename' && body.id) {
              const r = renameGiftFolder(body.id, body.name, home)
              sendJson(res, r.ok ? 200 : 404, r)
              return
            }
            if (action === 'folderDelete' && body.id) {
              const r = deleteGiftFolder(body.id, !!body.force, home)
              sendJson(res, r.ok ? 200 : (r.reason === 'not_found' ? 404 : 409), r)
              return
            }
            if (action === 'move') {
              sendJson(res, 200, moveGiftItems(body.ids, body.folderId, home))
              return
            }
            if (action === 'weight' && body.id) {
              const r = setGiftWeight(body.id, body.weight, home)
              sendJson(res, r.ok ? 200 : 404, r)
              return
            }
            if (action === 'delete' && body.id) {
              sendJson(res, 200, deleteGiftItems([body.id], home))
              return
            }
            if (action === 'deleteMany' && Array.isArray(body.ids)) {
              sendJson(res, 200, deleteGiftItems(body.ids, home))
              return
            }
            if (action === 'clearAll') {
              sendJson(res, 200, clearGiftAll(home))
              return
            }
            sendJson(res, 400, { error: 'unknown action' })
            return
    ...
    webServer.register({
      kind: 'exact',
      path: '/api/danmaku/gift/assets/upload',
      handler: async (req, res) => {
        try {
          if (req.method !== 'POST') {
            sendJson(res, 405, { error: 'POST required' })
          ...
          const url = new URL(req.url, 'http://127.0.0.1')
          const mb = Math.max(1, Math.min(64, Math.floor(Number(live.giftMaxAssetMB) || 8)))
          ...
          // (a) zip: raw binary archive, imported as a job (client polls status)
          if (kindParam === 'zip' || ct === 'application/zip') {
            if (giftJob && giftJob.running) { sendJson(res, 409, { ok: false, reason: 'busy' }); return }
            const body = await readCappedBody(req, maxBytes)
            if (!body.ok) {
              sendJson(res, body.reason === 'too_large' ? 413 : 400, { error: body.reason, limitMB: mb })
              return
            }
            startGiftZipBufferImport({ buffer: body.buffer, folderId, limit: 0 }).catch(() => {})
            sendJson(res, 200, { ok: true, started: true })
            return
          }

          // (b) pasted SVG source -> always through sanitizeSvg
          if (ct === 'application/x-svg-text' || ct === 'text/plain') {
            const body = await readCappedBody(req, maxBytes)
            if (!body.ok) {
              sendJson(res, body.reason === 'too_large' ? 413 : 400, { error: body.reason, limitMB: mb })
              return
            }
            const clean = sanitizeSvg(body.buffer.toString('utf8'))
            if (!clean.ok) {
              sendJson(res, 400, { error: clean.reason === 'too_large' ? 'svg too large' : 'invalid svg: ' + clean.reason })
              return
            }
            let name = sanitizeAssetName(rawName || 'pasted.svg')
            if (extname(name).toLowerCase() !== '.svg') {
              name = (name.replace(/\.[^.]*$/, '') || 'pasted') + '.svg'
            }
            const item = addGiftAsset({
              name, buffer: Buffer.from(clean.svg, 'utf8'), folderId, weight: 1, ext: '.svg', source: 'user',
            })
            if (!item) { sendJson(res, 400, { error: 'write failed' }); return }
            sendJson(res, 200, { ok: true, item })
            return
          }

          // (c) individual binary asset
          let name = sanitizeAssetName(rawName || ('asset' + (UPLOAD_CT_EXT[ct] || '')))
          let ext = extname(name).toLowerCase()
          if (!giftKindForExt(ext)) {
            const fallback = UPLOAD_CT_EXT[ct] || ''
            if (fallback) {
              ext = fallback
              name = (name.replace(/\.[^.]*$/, '') || 'asset') + ext
            }
          }
          if (!giftKindForExt(ext)) {
            sendJson(res, 415, { error: 'unsupported extension' })
            return
          }
          const body = await readCappedBody(req, maxBytes)
          if (!body.ok) {
          ...
          if (!body.buffer.length) { sendJson(res, 400, { error: 'empty body' }); return }
          ...
          const item = addGiftAsset({ name, buffer, folderId, weight: 1, ext, source: 'user' })
          if (!item) { sendJson(res, 400, { error: 'write failed' }); return }
          sendJson(res, 200, { ok: true, item })
        } catch (e) {
          sendJson(res, 500, { error: String(e && e.message ? e.message : e) })
    ...
    webServer.register({
      kind: 'exact',
      path: '/api/danmaku/gift/assets/import',
      handler: async (req, res) => {
        try {
          if (req.method !== 'POST') {
            sendJson(res, 405, { error: 'POST required' })
          ...
          if (body && body.action === 'abort') {
            abortGiftImport()
            sendJson(res, 200, { ok: true, status: getGiftImportStatus() })
            return
          }
          const kind = body && body.kind
          if (kind === 'git') {
    ...
    webServer.register({
      kind: 'exact',
      path: '/api/danmaku/gift/assets/import/status',
      handler: async (_req, res) => {
        try {
          sendJson(res, 200, getGiftImportStatus())
        } catch (e) {
          sendJson(res, 500, { error: String(e && e.message ? e.message : e) })
        }
      },
    ...
    webServer.register({
      kind: 'exact',
      path: '/api/danmaku/gift/catalog',
      handler: async (req, res) => {
        try {
          const home = process.env.DSH_HOME || join(homedir(), '.dsh')
          if (req.method === 'GET') {
            const payload = readGiftCatalog(home)
            if (!payload) {
              sendJson(res, 200, { ok: true, total: 0, source: 'none', gifts: [] })
              return
            }
            sendJson(res, 200, {
              ok: true,
          ...
          if (req.method === 'POST') {
            let body = {}
            try { body = await readBody(req) } catch { sendJson(res, 400, { ok: false, reason: 'bad-json' }); return }
            if (!body || body.action !== 'refresh') {
              sendJson(res, 400, { ok: false, reason: 'unknown-action' })
              return
            }
            const builtin = readGiftCatalogJson(giftBuiltinCatalogPath())
            const roomId = String(live.giftRoomId || '').replace(/[^0-9]/g, '')
            if (!roomId) {
              // No room configured → hand back the builtin catalog and write
              // NOTHING. (Reading it directly, not via readGiftCatalog, so the
              // no-room path never seeds a file.)
              sendJson(res, 200, {
                ok: false, reason: 'no-room', source: 'builtin',
                total: builtin ? builtin.gifts.length : 0, catalog: builtin || null,
              })
              return
            }
            try {
              const raw = await fetchLiveGiftCatalog(roomId)
              const gifts = raw.map((g) => mapCatalogGift(g))
              if (!gifts.length) throw new Error('empty catalog')
              const payload = {
                version: 1, generatedAt: new Date().toISOString(), source: 'live', via: 'giftConfig(pc)', gifts,
              }
              const wrote = writeGiftCatalogAtomic(payload, home)
              giftCatalogCache = null
              // Summary only — never the room id.
              console.log('[dsh-livechat] gift catalog refreshed: ' + gifts.length + ' gifts (wrote=' + wrote + ')')
              sendJson(res, 200, { ok: true, wrote, source: 'live', total: gifts.length, catalog: payload })
            } catch {
              // Silent degradation: builtin data, no error surfaced, no room id.
              sendJson(res, 200, {
                ok: false, reason: 'fetch-failed', source: 'builtin',
                total: builtin ? builtin.gifts.length : 0, catalog: builtin || null,
              })
            }
            return
