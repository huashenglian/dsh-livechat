...
function clampConfig(raw) {
  ...
  out.advancedStyles = styles.slice(0, 24).map((s, i) => {
    const mode = s && (s.mode === 'rain' || s.mode === 'pop') ? s.mode : 'scroll'
    return {
      ...
      mode,
      xPct: clampNum(s && s.xPct, 0, 100, 50),
      yPct: clampNum(s && s.yPct, 0, 100, 40),
    }
  })
  ...
  if (webServer) {
    webServer.register({
      ...
      handler: async (_req, res) => {
        sendJson(res, 200, { ok: true, name, version: '0.4.1' })
      },
    })
