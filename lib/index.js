...
function buildLlmPrompt(cfg, recent, events, extras) {
  ...
  if (extras && extras.thinking) {
    lines.push(`agent 最近思考片段（仅作氛围参考，勿照抄）：\n${String(extras.thinking).slice(0, 240)}`)
  }
  if (extras && extras.warning) {
...
function apply(ctx, _config) {
  ...
  const thinkingExcerpt = () => thinkingBuf.slice(-3).join(' ').slice(0, 240)
  ...
  ctx.on('dispose', () => {
    try {
      if (!pluginPackageStillInstalled()) {
        ...
        console.log('[dsh-livechat] uninstall cleanup, removed', r.removed, 'pool files')
      }
    } catch {
      /* ignore */
    }
      ...
      handler: async (req, res) => {
        try {
          ...
          const prompt = buildLlmPrompt(
            ...
            { thinking: thinkingExcerpt(), warning: lastWarningText },
          )
          let items = []
          try {
