  ...
  if (webServer) {
    webServer.register({
      ...
      handler: async (_req, res) => {
        sendJson(res, 200, { ok: true, name, version: '0.2.1' })
      },
    })
      ...
      handler: async (req, res) => {
        try {
          if (req.method === 'GET') {
            ...
            sendJson(res, 200, {
              items: m.items.map((x) => ({
                ...
                url: '/api/danmaku/emojis/image?id=' + encodeURIComponent(x.id),
              })),
            })
            return
          }
          if (req.method === 'PATCH' || req.method === 'POST') {
            const body = await readBody(req)
            if (body && body.action === 'delete' && body.id) {
              const m = loadEmojiManifest()
              const hit = m.items.find((x) => x.id === body.id)
              m.items = m.items.filter((x) => x.id !== body.id)
              saveEmojiManifest(m)
              if (hit && hit.file) {
                try { unlinkSync(join(emojiDir(), hit.file)) } catch { /* ignore */ }
              }
              sendJson(res, 200, { ok: true, items: m.items.length })
              return
            }
            if (body && body.action === 'weight' && body.id) {
              const m = loadEmojiManifest()
              const it = m.items.find((x) => x.id === body.id)
              if (!it) {
                sendJson(res, 404, { error: 'not found' })
                return
              }
              it.weight = Math.max(0, Number(body.weight) || 0)
              saveEmojiManifest(m)
              sendJson(res, 200, { ok: true, weight: it.weight })
              return
            }
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          const name = String(body?.name || 'emoji').slice(0, 32)
          ...
          const file = id + '.' + ext
          mkdirSync(emojiDir(), { recursive: true })
          writeFileSync(join(emojiDir(), file), Buffer.from(m[2], 'base64'))
          const manifest = loadEmojiManifest()
          manifest.items.push({ id, name, file, weight })
          saveEmojiManifest(manifest)
          sendJson(res, 200, {
            ok: true,
            item: { id, name, weight, url: '/api/danmaku/emojis/image?id=' + encodeURIComponent(id) },
          })
        } catch (e) {
          sendJson(res, 500, { error: String(e && e.message ? e.message : e) })
        }
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          const ext = (it.file.split('.').pop() || 'png').toLowerCase()
