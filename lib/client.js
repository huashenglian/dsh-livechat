	...
	factory: (require) => {
		...
		function createWebgl2WorkerRenderer() {
			...
			var WORKER_SRC = [
				...
				'if(kind==="sc"){ctx.fillStyle="rgba(251,114,153,0.95)";ctx.fillRect(0,0,Math.min(tw,CW),Math.min(th,CH));ctx.fillStyle="#fff";ctx.fillText(text,pad,Math.min(th,CH)/2);}else{if(stroke){ctx.lineWidth=Math.max(2,fs*0.12);ctx.lineJoin="round";ctx.strokeStyle="#000";ctx.strokeText(text,pad,th/2);}ctx.fillStyle="#fff";ctx.fillText(text,pad,th/2);}',
