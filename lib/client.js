			...
			React.useEffect(function () {
				...
				function schedule() {
					...
					timer = setTimeout(function () {
						...
						// Random gift dice (todo 20): reuses THIS timer (no second
						// timer). Independent draw; participates only when
						// giftEnabled && giftTrigger.random. Deliberately above the
						// session guard so the gift simulator is not coupled to a
						// conversation being open (the danmaku ambient below stays
						// session-gated).
