window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function GiftConfigModal(props) {
			...
			var prob = Number(trig.probability)
			if (!Number.isFinite(prob)) prob = 0
			prob = Math.min(1, Math.max(0, prob))
			var minS = Math.max(1, Math.round((Number(trig.minMs) || 1000) / 1000))
			var maxS = Math.max(minS, Math.round((Number(trig.maxMs) || minS * 1000) / 1000))
			...
			var writeChain = React.useRef(Promise.resolve())
			// 重置绑定 remounts the (uncontrolled) binding panel by bumping this key.
			var bindTickState = React.useState(0)
			var bindTick = bindTickState[0]
			var setBindTick = bindTickState[1]
			...
			// Synchronous accumulating ref so rapid slider edits each build on the
			// previous one instead of a stale render snapshot.
			var trigRef = React.useRef(trig)
			React.useEffect(function () { trigRef.current = trig }, [trig])
			function patchTrigger(ov) {
				trigRef.current = Object.assign({}, trigRef.current, ov)
				return writeCfg({ giftTrigger: trigRef.current })
			}
			function onTemplateChange(v) { setTpl(v); writeCfg({ giftTemplate: v }) }
			// 重置绑定 (todo 26): clear EVERY gift binding. A delta POST — the host
			// merges, so only `giftBindings` is replaced. The binding panel is
			// uncontrolled, so bump the key below to remount it with empty state.
			function resetBindings() {
				writeCfg({ giftBindings: {} })
					.then(function () { setBindTick(function (n) { return n + 1 }) })
					.catch(function () { /* errText already set by writeCfg */ })
				var c = readCfg()
				if (c) setCfg(Object.assign({}, c, { giftBindings: {} }))
				onToast('success', '已重置绑定', '')
			}
			var sampleUser = ((cfg && cfg.giftSenders && cfg.giftSenders[0] && cfg.giftSenders[0].name) || '匿名')
			var templatePreview = renderGiftTip(tpl, sampleUser, (selected && selected.name) || '小电视')
					...
					React.createElement('div', { key: 'b', className: 'dsh-gift-modal-body' }, [
						...
						React.createElement('div', { key: 'r', className: 'dsh-gift-modal-right', ref: rightRef, 'data-gift-modal-right': '1' }, [
							...
							React.createElement('div', {
								...
								React.createElement('button', {
									key: 'rb', type: 'button', className: 'dsh-gift-reset-bindings',
									'data-gift-reset-bindings': '1', onClick: resetBindings,
								}, '重置绑定'),
								React.createElement(GiftBindingPanel, { key: 'bind' + bindTick, gift: selected, onToast: onToast }),
							]),
							React.createElement('div', {
								...
								React.createElement('div', { key: 'sim', className: 'dsh-gift-sim', 'data-gift-sim': '1' }, [
									...
									React.createElement('div', { key: 'tm', className: 'dsh-gift-sim-row' }, [
										React.createElement('label', { key: 'man' }, [
											React.createElement('input', {
												key: 'i', type: 'checkbox', 'data-gift-trigger-manual': '1', checked: trig.manual === true,
												onChange: function (e) { patchTrigger({ manual: e.target.checked }) },
											}),
											React.createElement('span', { key: 'l' }, ' ' + t('giftTriggerManual')),
										]),
										React.createElement('label', { key: 'rnd' }, [
											React.createElement('input', {
												key: 'i', type: 'checkbox', 'data-gift-trigger-random': '1', checked: trig.random === true,
												onChange: function (e) { patchTrigger({ random: e.target.checked }) },
											}),
											React.createElement('span', { key: 'l' }, ' ' + t('giftTriggerRandom')),
										]),
									]),
									React.createElement('div', { key: 'tp', className: 'dsh-gift-sim-row' }, [
										React.createElement('span', { key: 'l' }, t('giftTriggerProb')),
										React.createElement('input', {
											key: 'i', type: 'range', min: 0, max: 1, step: 0.01,
											'data-gift-trigger-prob': '1', value: prob,
											onChange: function (e) { patchTrigger({ probability: Number(e.target.value) }) },
										}),
										React.createElement('span', { key: 'v', 'data-gift-trigger-prob-val': '1' }, prob.toFixed(2)),
									]),
									React.createElement('div', { key: 'ti', className: 'dsh-gift-sim-row' }, [
										React.createElement('span', { key: 'mn' }, t('giftTriggerMin')),
										React.createElement('input', {
											key: 'min', type: 'range', min: 1, max: 3600, step: 1,
											'data-gift-trigger-min': '1', value: minS,
											onChange: function (e) { patchTrigger({ minMs: Math.max(1, Math.round(Number(e.target.value)) || 1) * 1000 }) },
										}),
										React.createElement('span', { key: 'mnv', 'data-gift-trigger-min-val': '1' }, minS + 's'),
										React.createElement('span', { key: 'mx' }, t('giftTriggerMax')),
										React.createElement('input', {
											key: 'max', type: 'range', min: 1, max: 3600, step: 1,
											'data-gift-trigger-max': '1', value: maxS,
											onChange: function (e) { patchTrigger({ maxMs: Math.max(1, Math.round(Number(e.target.value)) || 1) * 1000 }) },
										}),
										React.createElement('span', { key: 'mxv', 'data-gift-trigger-max-val': '1' }, maxS + 's'),
									]),
