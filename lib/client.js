	...
	factory: (require) => {
		...
		function ensureStyles() {
			...
			var css = [
				...
				'.dsh-danmaku-modal-card{width:min(360px,90vw);border-radius:14px;background:var(--dsw-alias-bg-layer-2,#fff);color:var(--dsw-alias-label-primary,#111);padding:16px;border:1px solid var(--dsw-alias-border-l3,rgba(0,0,0,.12));box-shadow:0 12px 40px var(--dsw-alias-bg-mask-2,rgba(0,0,0,.2));}',
				...
				'@keyframes dsh-danmaku-rain{from{transform:translate3d(0,-40px,0);}to{transform:translate3d(var(--dsh-danmaku-dx),calc(100vh + 40px),0);}}',
		...
		function createDomRenderer() {
			...
			function spawnOne(item) {
				...
				var y
				if (layout === 'roll') {
					// area_ratio only limits roll tracks
					y = track * lh + 2
				} else if (layout === 'top') {
					y = track * lh + 2
				} else {
				...
				el.addEventListener('animationend', function (e) {
					if (e.animationName === 'dsh-danmaku-fade' || e.animationName === 'dsh-danmaku-roll' || e.animationName === 'dsh-danmaku-rain') {
						tryRemoveDom()
					}
				})
			...
			function applyIdle() {
				items.forEach(function (rec) {
					...
					if (textEl) textEl.style.opacity = String(config.opacity * idleAlpha)
				})
			}
			...
			function applyScrollSpeedLive(newSpeed, oldSpeed) {
				...
				items.forEach(function (rec) {
					if (rec.el.dataset.layout !== 'roll') return
					try {
						...
						items.forEach(function (rec) {
							...
							if (textEl) {
								textEl.style.opacity = String(
									(rec.el.dataset.kind === 'sc' ? 1 : config.opacity) * idleAlpha
								)
							}
						})
					}
		...
		function createWebgl2WorkerRenderer() {
			...
			function pickTrack(layout, widthCss, fontSize, speed) {
				...
				var now = performance.now()
				for (var i = 0; i < n; i++) {
					if (free[i] == null || now >= free[i]) return i
				}
				return -1
			}

			function markTrack(layout, index, widthCss, speed, now) {
				var gapMs = ((widthCss + 12) / Math.max(speed, 40)) * 1000
				if (!trackFreeAt[layout]) trackFreeAt[layout] = []
				trackFreeAt[layout][index] = now + gapMs
			}
			...
			var WORKER_SRC = [
				...
				'const idata=new Float32Array(list.length*INST_FLOATS);const udata=new Float32Array(list.length*8);list.forEach((r,i)=>{const o=i*INST_FLOATS;idata[o]=r.t0*0.001;idata[o+1]=r.durationMs*0.001;idata[o+2]=r.paused?r.pausedBiasSec:0;idata[o+3]=r.layout==="roll"?0:1;idata[o+4]=r.x0;idata[o+5]=r.y;idata[o+6]=r.w;idata[o+7]=r.h;idata[o+8]=r.x1;const uo=i*8;udata[uo]=r.u0;udata[uo+1]=r.v0;udata[uo+2]=r.u1;udata[uo+3]=r.v1;udata[uo+4]=r.alpha;udata[uo+5]=r.tr;udata[uo+6]=r.tg;udata[uo+7]=r.tb;});',
				...
				'if(m.type==="styles"){const op=m.opacity;const fs=m.fontSize;const sp=m.scrollSpeed;if(op!=null)config.opacity=op;if(fs)config.fontSize=fs;if(sp)config.scrollSpeed=sp;const now=performance.now();insts.forEach((r)=>{if(r.kind==="sc")return;if(op!=null)r.alpha=op;if(r.layout==="roll"&&sp){const t=r.paused?r.pausedProgress:(now-r.t0)/Math.max(r.durationMs,1);const wCss=(r.w||100)/Math.max(dpr,0.5);const cssWpx=(canvas.width||1)/Math.max(dpr,0.5);const nd=Math.max(2000,Math.min(20000,((cssWpx+wCss+24)/Math.max(40,sp))*1000));r.durationMs=nd;r.t0=now-t*nd;r.x1=-((r.w||100)+24);}if(fs&&r.content){const b=bake(r.content,r.kind,fs,true);r.u0=b.u0;r.v0=b.v0;r.u1=b.u1;r.v1=b.v1;r.w=b.w;r.h=b.h;}});return;}',
				'if(m.type==="pause-all"){const now=performance.now();insts.forEach((r)=>{if(!r.paused){const p=Math.min(1,Math.max(0,(now-r.t0)/Math.max(r.durationMs,1)));r.pausedProgress=p;r.pausedBiasSec=p*(r.durationMs*0.001)-(now-r.t0)*0.001;r.paused=true;}});return;}',
				'if(m.type==="resume-all"){const now=performance.now();insts.forEach((r)=>{if(r.paused){const p=Math.min(1,Math.max(0,r.pausedProgress||0));r.t0=now-p*r.durationMs;r.pausedBiasSec=0;r.paused=false;}});return;}',
				...
				'if(m.type==="debug-snapshot"){const now=performance.now();const out=[];insts.forEach((r,id)=>{const t=r.paused?r.pausedProgress:(now-r.t0)/Math.max(r.durationMs,1);const x=r.layout==="roll"?(r.x0+(r.x1-r.x0)*Math.min(1,Math.max(0,t))):((canvas.width-r.w)*0.5);out.push({id,paused:r.paused,t,x:x/dpr});});self.postMessage({type:"debug-snapshot",items:out});return;}',
			...
			function spawnOne(item) {
				...
				if (track < 0) return
				var now = performance.now()
				markTrack(layout, track, wCss, speed, now)
				...
				var x1 = layout === 'roll' ? -(wCss * dpr + 24) : 0
				var cr = 1, cg = 1, cb = 1
				if (kind !== 'sc' && color && color.charAt(0) === '#') {
					var n = parseInt(color.slice(1), 16)
					cr = ((n >> 16) & 255) / 255
		...
		function ControlBall(props) {
			...
			var popOpen = popState[0]
			var setPopOpen = popState[1]
			...
			var pop = React.createElement('div', {
				...
				React.createElement('label', { key: 'o' }, [
					...
					React.createElement('input', {
						key: 'i', type: 'range', min: 0.1, max: 1, step: 0.05, value: config.opacity,
						onChange: function (e) { patchLive({ opacity: Number(e.target.value) }) }
					})
				]),
				React.createElement('label', { key: 'sp' }, [
					React.createElement('span', { key: 'l' }, t('scrollSpeed')),
					React.createElement('input', {
						key: 'i', type: 'range', min: 40, max: 320, step: 10, value: config.scrollSpeed || 140,
						onChange: function (e) { patchLive({ scrollSpeed: Number(e.target.value) }) }
					})
				]),
				React.createElement('label', { key: 'fs' }, [
					React.createElement('span', { key: 'l' }, t('fontSize')),
					React.createElement('input', {
						key: 'i', type: 'range', min: 12, max: 28, step: 1, value: config.fontSize,
						onChange: function (e) { patchLive({ fontSize: Number(e.target.value) }) }
					})
				]),
				React.createElement('label', { key: 'm' }, [
					React.createElement('span', { key: 'l' }, t('maxOnscreen')),
					React.createElement('input', {
						key: 'i', type: 'range', min: 5, max: 80, step: 1, value: config.maxOnscreen,
						onChange: function (e) { patchLive({ maxOnscreen: Number(e.target.value) }) }
					})
				]),
				...
				React.createElement('div', {
					...
					weights.map(function (w, i) {
						return React.createElement('div', {
							...
							React.createElement('span', { key: 'v', style: { width: 28, textAlign: 'right' } }, String(w.weight || 0)),
							React.createElement('button', {
								key: 'x',
		...
		function SettingsPanel(props) {
				...
				fetchJson('/api/danmaku/models').then(function (d) {
					...
					;((d && d.groups) || []).forEach(function (g) {
						;(g.models || []).forEach(function (m) {
							list.push({ id: g.provider + '/' + m.id, label: g.provider + ' · ' + (m.name || m.id) })
						})
					})
					setModels(list)
			...
			return React.createElement('div', { className: 'dsh-danmaku-card' }, [
				...
				React.createElement('div', { className: 'dsh-danmaku-section', key: 's1' }, t('secBasics')),
				...
				row('opacity', t('opacity') + ' ' + Number(draft.opacity).toFixed(2), React.createElement('input', {
					key: 'c', type: 'range', min: 0.1, max: 1, step: 0.05, value: draft.opacity,
					onChange: function (e) { patch({ opacity: Number(e.target.value) }) }
				})),
				row('opacityIdle', t('opacityIdle') + ' ' + Number(draft.opacityIdle).toFixed(2), React.createElement('input', {
					key: 'c', type: 'range', min: 0, max: 0.6, step: 0.05, value: draft.opacityIdle,
					onChange: function (e) { patch({ opacityIdle: Number(e.target.value) }) }
				})),
				...
				row('maxOnscreen', t('maxOnscreen'), React.createElement('input', {
					key: 'c', type: 'number', min: 1, max: 200, value: draft.maxOnscreen,
					onChange: function (e) { patch({ maxOnscreen: Number(e.target.value) }) }
				})),
				row('fontSize', t('fontSize'), React.createElement('input', {
					key: 'c', type: 'range', min: 12, max: 28, value: draft.fontSize,
					onChange: function (e) { patch({ fontSize: Number(e.target.value) }) }
				})),
				row('crossSec', t('crossSec'), React.createElement('input', {
					key: 'c', type: 'range', min: 4, max: 15, value: draft.crossSec,
					onChange: function (e) { patch({ crossSec: Number(e.target.value) }) }
				})),
				row('scrollSpeed', t('scrollSpeed') + ' ' + (draft.scrollSpeed || 140), React.createElement('input', {
					key: 'c', type: 'range', min: 40, max: 320, step: 10, value: draft.scrollSpeed || 140,
					onChange: function (e) { patch({ scrollSpeed: Number(e.target.value) }) }
				})),
				...
				row('layoutRoll', t('layoutRoll'), React.createElement('input', {
					key: 'c', type: 'number', min: 0, max: 100, value: draft.layoutWeights.roll,
					onChange: function (e) { patch({ layoutWeights: Object.assign({}, draft.layoutWeights, { roll: Number(e.target.value) }) }) }
				})),
				row('layoutTop', t('layoutTop'), React.createElement('input', {
					key: 'c', type: 'number', min: 0, max: 100, value: draft.layoutWeights.top,
					onChange: function (e) { patch({ layoutWeights: Object.assign({}, draft.layoutWeights, { top: Number(e.target.value) }) }) }
				})),
				row('layoutBottom', t('layoutBottom'), React.createElement('input', {
					key: 'c', type: 'number', min: 0, max: 100, value: draft.layoutWeights.bottom,
					onChange: function (e) { patch({ layoutWeights: Object.assign({}, draft.layoutWeights, { bottom: Number(e.target.value) }) }) }
				})),
				...
				React.createElement('div', { className: 'dsh-danmaku-section', key: 's2' }, t('secContent')),
				...
				row('emojiBaseSize', t('emojiBaseSize') + ' ' + (draft.emojiBaseSize || 48), React.createElement('input', {
					key: 'c', type: 'range', min: 16, max: 128, step: 4, value: draft.emojiBaseSize || 48,
					disabled: draft.emojiEnabled === false,
					onChange: function (e) { patch({ emojiBaseSize: Number(e.target.value) }) }
				})),
				row('emojiTotalProb', t('emojiTotalProb') + ' ' + Number(draft.emojiTotalProb != null ? draft.emojiTotalProb : 0.08).toFixed(2), React.createElement('input', {
					key: 'c', type: 'range', min: 0, max: 0.5, step: 0.01, value: draft.emojiTotalProb != null ? draft.emojiTotalProb : 0.08,
					disabled: draft.emojiEnabled === false,
					onChange: function (e) { patch({ emojiTotalProb: Number(e.target.value) }) }
				})),
				...
				row('density', t('density') + ' ' + (draft.density || 3), React.createElement('input', {
					key: 'c', type: 'range', min: 1, max: 5, step: 1, value: draft.density || 3,
					onChange: function (e) { patch({ density: Number(e.target.value) }) }
				})),
				row('maxPoolEntries', t('maxPoolEntries'), React.createElement('input', {
					key: 'c', type: 'number', min: 20, max: 2000, value: draft.maxPoolEntries || 200,
					onChange: function (e) { patch({ maxPoolEntries: Number(e.target.value) }) }
				})),
				row('historyReplayMax', t('historyReplayMax'), React.createElement('input', {
					key: 'c', type: 'number', min: 0, max: 50, value: draft.historyReplayMax != null ? draft.historyReplayMax : 12,
					onChange: function (e) { patch({ historyReplayMax: Number(e.target.value) }) }
				})),
				row('blockedWords', t('blockedWords'), React.createElement('input', {
					key: 'c', type: 'text', value: (draft.blockedWords || []).join(', '),
					onChange: function (e) {
						patch({ blockedWords: e.target.value.split(/[,，]/).map(function (s) { return s.trim() }).filter(Boolean) })
					}
				})),
				...
				React.createElement('div', { className: 'dsh-danmaku-section', key: 'sColor' }, t('secFx')),
				...
				row('halfLifeDays', t('halfLifeDays'), React.createElement('input', {
					key: 'c', type: 'number', min: 1, max: 30, value: draft.halfLifeDays || 7,
					disabled: draft.historyReplayEnabled === false,
					onChange: function (e) { patch({ halfLifeDays: Number(e.target.value) }) }
				})),
				row('archiveAfterDays', t('archiveAfterDays'), React.createElement('input', {
					key: 'c', type: 'number', min: 1, max: 365, value: draft.archiveAfterDays || 60,
					onChange: function (e) { patch({ archiveAfterDays: Number(e.target.value) }) }
				})),
				row('restoreMaxAgeHours', t('restoreMaxAgeHours') + ' (h)', React.createElement('input', {
					key: 'c', type: 'number', min: 1, max: 720, value: draft.restoreMaxAgeHours || 168,
					onChange: function (e) { patch({ restoreMaxAgeHours: Number(e.target.value) }) }
				})),
				...
				React.createElement('div', { className: 'dsh-danmaku-section', key: 's3' }, t('secLlm')),
				...
				row('llmModel', t('llmModel'), models && models.length
					? React.createElement('select', {
						key: 'c', value: draft.llmModel || '-',
						onChange: function (e) { patch({ llmModel: e.target.value }) }
					}, [React.createElement('option', { key: '-', value: '-' }, '-')].concat(models.map(function (m) {
				...
				(draft.wakeupType === 'smart' || !draft.wakeupType) ? row('wakeupEvents', t('wakeupEvents'), React.createElement('input', {
					key: 'c', type: 'text', value: (draft.wakeupEvents || []).join(', '),
					onChange: function (e) {
						patch({ wakeupEvents: e.target.value.split(/[,，]/).map(function (s) { return s.trim() }).filter(Boolean) })
					}
				})) : null,
				(draft.wakeupType === 'smart' || !draft.wakeupType) ? row('smartMinGapSec', t('smartMinGapSec'), React.createElement('input', {
					key: 'c', type: 'number', min: 3, max: 60, value: draft.smartMinGapSec || 8,
					onChange: function (e) { patch({ smartMinGapSec: Number(e.target.value) }) }
				})) : null,
				...
				draft.wakeupType === 'toolcall' ? row('toolcallTools', t('toolcallTools'), React.createElement('input', {
					key: 'c', type: 'text', value: (draft.toolcallTools || []).join(', '),
					onChange: function (e) {
						patch({ toolcallTools: e.target.value.split(/[,，]/).map(function (s) { return s.trim() }).filter(Boolean) })
					}
				})) : null,
				...
				row('globalMinGapSec', t('globalMinGapSec'), React.createElement('input', {
					key: 'c', type: 'number', min: 1, max: 60, value: draft.globalMinGapSec || 5,
					onChange: function (e) { patch({ globalMinGapSec: Number(e.target.value) }) }
				})),
				React.createElement(CollapseSection, {
					...
					row('backoffBaseSec', t('backoffBaseSec'), React.createElement('input', {
						key: 'c', type: 'number', min: 5, max: 120, value: draft.backoffBaseSec || 12,
						disabled: draft.backoffEnabled === false,
						onChange: function (e) { patch({ backoffBaseSec: Number(e.target.value) }) }
					})),
					row('backoffMaxSec', t('backoffMaxSec'), React.createElement('input', {
						key: 'c', type: 'number', min: 10, max: 300, value: draft.backoffMaxSec || 60,
						disabled: draft.backoffEnabled === false,
						onChange: function (e) { patch({ backoffMaxSec: Number(e.target.value) }) }
					})),
					row('backoffFactor', t('backoffFactor'), React.createElement('input', {
						key: 'c', type: 'number', min: 1.1, max: 3, step: 0.1, value: draft.backoffFactor || 1.5,
						disabled: draft.backoffEnabled === false,
						onChange: function (e) { patch({ backoffFactor: Number(e.target.value) }) }
					})),
					row('backoffResetSec', t('backoffResetSec'), React.createElement('input', {
						key: 'c', type: 'number', min: 2, max: 60, value: draft.backoffResetSec || 6,
						disabled: draft.backoffEnabled === false,
						onChange: function (e) { patch({ backoffResetSec: Number(e.target.value) }) }
					})),
					...
					row('thinkingExcerptChars', t('thinkingExcerptChars'), React.createElement('input', {
						key: 'c', type: 'number', min: 80, max: 480, step: 20, value: draft.thinkingExcerptChars || 240,
						onChange: function (e) { patch({ thinkingExcerptChars: Number(e.target.value) }) }
					}))
				]),
				React.createElement(CollapseSection, {
					key: 'ctxSrc',
				...
				row('llmIntervalSec', t('llmIntervalSec'), React.createElement('input', {
					key: 'c', type: 'number', min: 5, max: 120, value: draft.llmIntervalSec,
					disabled: !draft.llmEnabled,
					onChange: function (e) { patch({ llmIntervalSec: Number(e.target.value) }) }
				})),
				row('llmBurstCount', t('llmBurstCount'), React.createElement('input', {
					key: 'c', type: 'number', min: 1, max: 20, value: draft.llmBurstCount,
					disabled: !draft.llmEnabled,
					onChange: function (e) { patch({ llmBurstCount: Number(e.target.value) }) }
				})),
				row('stylePrompt', t('stylePrompt'), React.createElement('input', {
					key: 'c', type: 'text', value: draft.stylePrompt || '',
					onChange: function (e) { patch({ stylePrompt: e.target.value }) }
				})),
				React.createElement('div', {
					key: 'tpl',
				...
				row('styleMaxChars', t('styleMaxChars'), React.createElement('input', {
					key: 'c', type: 'number', min: 8, max: 48, value: draft.styleMaxChars || 24,
					onChange: function (e) { patch({ styleMaxChars: Number(e.target.value) }) }
				})),
				row('matchThreshold', t('matchThreshold') + ' ' + Number(draft.matchThreshold != null ? draft.matchThreshold : 0.35).toFixed(2), React.createElement('input', {
					key: 'c', type: 'range', min: 0, max: 1, step: 0.05, value: draft.matchThreshold != null ? draft.matchThreshold : 0.35,
					onChange: function (e) { patch({ matchThreshold: Number(e.target.value) }) }
				})),
				React.createElement('div', { className: 'dsh-danmaku-hr', key: 'hr-render' }),
				React.createElement('div', { className: 'dsh-danmaku-section', key: 'sRender' }, t('secRender')),
		...
		function TemplatesEditor(props) {
			...
			return React.createElement('div', { className: 'dsh-danmaku-modal', onClick: onClose }, [
				React.createElement('div', {
					key: 'card',
					className: 'dsh-danmaku-modal-card',
		...
		function PoolEditorModal(props) {
			...
			function clearAllPools() {
				...
				}).catch(function () { setMsg(t('poolClearFail')) })
			}
			...
			return React.createElement('div', { className: 'dsh-danmaku-pool', onClick: onClose }, [
				React.createElement('div', {
					key: 'box',
					...
					React.createElement('div', { key: 'b', className: 'dsh-danmaku-pool-body' }, [
						React.createElement('div', { key: 's', className: 'dsh-danmaku-pool-sessions' }, [
							...
							return React.createElement('button', {
								...
								React.createElement('div', { key: 'i', style: { fontFamily: 'monospace', fontSize: 11 } }, s.sessionId.slice(0, 18) + '…'),
									...
									React.createElement('tbody', { key: 'tb' }, items.map(function (item) {
										...
										return React.createElement('tr', { key: item.id }, [
											...
											React.createElement('td', { key: 'w' }, [
												React.createElement('input', {
													key: 'i', type: 'number', min: 0, max: 10, step: 0.5, defaultValue: item.weight || 1, style: { width: 70 },
													onChange: function (e) {
														draftsRef.current[item.id] = Object.assign({}, draftsRef.current[item.id], { weight: Number(e.target.value) })
													}
												})
											]),
				...
				clearConfirm ? React.createElement('div', {
					key: 'clear-confirm',
					className: 'dsh-danmaku-modal',
					style: { zIndex: 10040 },
		...
		function DanmakuSettingsModal(props) {
			...
			var tabs = [
				{ id: 'basics', label: t('modalBasics'), anchor: t('secBasics') },
				{ id: 'content', label: t('modalContent'), anchor: t('secFx') },
				{ id: 'llm', label: t('modalLlm'), anchor: t('secLlm') },
				{ id: 'pool', label: t('modalPool'), anchor: t('secContent') },
				{ id: 'render', label: t('modalRender'), anchor: t('secRender') }
			]

			function scrollTo(anchor) {
				var root = mainRef.current
				if (!root) return
				var nodes = root.querySelectorAll('.dsh-danmaku-section, .dsh-danmaku-card h3')
				for (var i = 0; i < nodes.length; i++) {
					if (String(nodes[i].textContent || '').indexOf(anchor) >= 0) {
		...
		function PresetEditorModal(props) {
			...
			return React.createElement('div', { className: 'dsh-danmaku-modal', onClick: onClose }, [
				React.createElement('div', {
					key: 'box',
					className: 'dsh-danmaku-modal-card',
					style: { width: 'min(640px,94vw)', maxHeight: '86vh', display: 'flex', flexDirection: 'column' },
					onClick: function (e) { e.stopPropagation() }
				}, [
					React.createElement('h3', { key: 'h' }, t('presetsTitle')),
					React.createElement('div', { key: 'row', style: { display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' } }, [
						React.createElement('select', {
							key: 'sel',
							value: selected,
							style: { flex: 1, minWidth: 120, padding: 6, borderRadius: 8 },
					...
					pack ? React.createElement('input', {
						...
						style: { width: '100%', marginBottom: 8, padding: 8, borderRadius: 8, border: '1px solid var(--dsw-alias-border-l3,rgba(0,0,0,.15))' },
						onChange: function (e) {
							var next = {
								packs: Object.assign({}, store.packs),
								order: store.order
					...
					pack ? React.createElement('textarea', {
						...
						value: linesText,
						rows: 12,
						style: { width: '100%', fontFamily: 'monospace', fontSize: 12, borderRadius: 8, padding: 8, border: '1px solid var(--dsw-alias-border-l3,rgba(0,0,0,.15))', resize: 'vertical' },
		...
		function EmojiEditorModal(props) {
			...
			function reload() {
				fetchJson('/api/danmaku/emojis').then(function (d) {
					setItems((d && d.items) || [])
				}).catch(function () { setItems([]) })
			}
			...
			function onFile(e) {
				...
				reader.onload = function () {
					fetchJson('/api/danmaku/emojis/upload', {
						method: 'POST',
						headers: { 'Content-Type': 'application/json' },
						body: JSON.stringify({ name: f.name.replace(/\.[^.]+$/, ''), dataUrl: reader.result, weight: 1 })
					}).then(function () {
						setMsg(t('saved'))
						reload()
					}).catch(function () { setMsg(t('saveFail')) })
			...
			return React.createElement('div', { className: 'dsh-danmaku-modal', onClick: onClose }, [
				React.createElement('div', {
					key: 'box',
					className: 'dsh-danmaku-modal-card',
					style: { width: 'min(560px,94vw)', maxHeight: '86vh', display: 'flex', flexDirection: 'column' },
					onClick: function (e) { e.stopPropagation() }
				}, [
					React.createElement('h3', { key: 'h' }, t('emojiTitle')),
					React.createElement('div', { key: 'hint', style: { fontSize: 12, opacity: 0.7, marginBottom: 8 } }, t('emojiHint')),
					React.createElement('div', { key: 'imp', style: { marginBottom: 10 } }, [
						React.createElement('input', {
					...
					items.length === 0
						? React.createElement('div', { key: 'e', style: { opacity: 0.6 } }, t('emojiEmpty'))
						: React.createElement('div', { key: 'list', style: { overflow: 'auto' } }, items.map(function (it) {
							return React.createElement('div', {
								key: it.id,
								style: { display: 'flex', gap: 10, alignItems: 'center', padding: '6px 0', borderBottom: '1px solid rgba(128,128,128,.15)' }
							}, [
								React.createElement('img', {
									key: 'i', src: it.url, alt: it.name,
									style: { width: 36, height: 36, objectFit: 'contain', borderRadius: 6 }
								}),
								React.createElement('span', { key: 'n', style: { flex: 1, fontSize: 12 } }, it.name),
								React.createElement('label', { key: 'w', style: { fontSize: 12 } }, t('emojiWeight')),
								React.createElement('input', {
									key: 'wi', type: 'number', min: 0, max: 100, step: 0.5, value: it.weight,
									style: { width: 64 },
									onChange: function (e) {
										var w = Number(e.target.value)
										setItems(items.map(function (x) { return x.id === it.id ? Object.assign({}, x, { weight: w }) : x }))
										fetchJson('/api/danmaku/emojis', {
											method: 'PATCH',
											headers: { 'Content-Type': 'application/json' },
											body: JSON.stringify({ action: 'weight', id: it.id, weight: w })
										}).catch(function () {})
									}
								}),
								React.createElement('button', {
		...
		function DanmakuOverlay(props) {
			...
			function enqueue(items, source) {
				var arr = (items || []).map(function (it) {
					return {
						...
						color: it.color || ((it.source || source) === 'ai' ? '#89D5FF' : undefined),
						kind: it.kind || 'normal',
						at: it.at,
						tags: it.tags
					}
					...
					timer = setTimeout(function () {
						...
						if (queueRef.current.length > 0) {
							...
							rendererRef.current.spawn([{
								content: wp.content,
								source: wp.source,
								color: wp.source === 'ai' ? '#89D5FF' : undefined,
			...
			var modal = detail ? React.createElement('div', {
				key: 'modal',
				className: 'dsh-danmaku-modal',
			...
			return React.createElement('div', { className: 'dsh-danmaku-root', ref: hostRef }, [
