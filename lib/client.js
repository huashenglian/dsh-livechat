	...
	factory: (require) => {
		...
		function createWebgl2Renderer(opts) {
			...
			var FS = [
				...
				'  vec3 col = mix(vec3(0.02), vTint, fill);',
			...
			function rebuildInstances(now) {
				...
				items.forEach(function (rec, id) {
					...
					instData[o + 6] = rec.w
					instData[o + 7] = rec.h
					// aEndX: x1, paused flag, pausedProgress, unused
					instData[o + 8] = rec.x1
		...
		function createWebgl2WorkerRenderer() {
			...
			var WORKER_SRC = [
				...
				'const VS=`#version 300 es\\nin vec2 aPos;in vec4 aTime;in vec4 aRect;in vec4 aEndX;in vec4 aUV;in vec4 aAlphaCol;uniform float uTime;uniform vec2 uViewport;uniform float uIdleAlpha;out vec2 vUV;out float vAlpha;out vec3 vTint;void main(){float dur=max(aTime.y,0.001);float t=(aEndX.y>0.5)?aEndX.z:clamp((uTime-aTime.x+aTime.z)/dur,0.0,1.0);float mode=aTime.w;float x=(mode<0.5)?mix(aRect.x,aEndX.x,t):((uViewport.x-aRect.z)*0.5);vec2 p=vec2(x,aRect.y)+aPos*aRect.zw;vec2 clip=vec2(p.x/uViewport.x*2.0-1.0,1.0-p.y/uViewport.y*2.0);gl_Position=vec4(clip,0.0,1.0);vUV=mix(aUV.xy,aUV.zw,aPos);float fade=1.0;if(t<0.06)fade=t/0.06;else if(t>0.94)fade=(1.0-t)/0.06;vAlpha=aAlphaCol.x*fade*uIdleAlpha;vTint=aAlphaCol.yzw;}`;',
				...
				'const idata=new Float32Array(list.length*INST_FLOATS);const udata=new Float32Array(list.length*8);list.forEach((r,i)=>{const o=i*INST_FLOATS;idata[o]=(r.t0-t0)*0.001;idata[o+1]=r.durationMs*0.001;idata[o+2]=r.paused?r.pausedBiasSec:0;idata[o+3]=r.layout==="roll"?0:1;idata[o+4]=r.x0;idata[o+5]=r.y;idata[o+6]=r.w;idata[o+7]=r.h;idata[o+8]=r.x1;idata[o+9]=r.paused?1:0;idata[o+10]=r.paused?r.pausedProgress:0;idata[o+11]=0;const uo=i*8;udata[uo]=r.u0;udata[uo+1]=r.v0;udata[uo+2]=r.u1;udata[uo+3]=r.v1;udata[uo+4]=r.alpha;udata[uo+5]=r.tr;udata[uo+6]=r.tg;udata[uo+7]=r.tb;});',
				...
				'if(m.type==="styles"){const op=m.opacity;const fs=m.fontSize;const sp=m.scrollSpeed;const st=m.stroke;if(op!=null)config.opacity=op;if(fs)config.fontSize=fs;if(sp)config.scrollSpeed=sp;if(st!=null)config.stroke=st;const now=performance.now();insts.forEach((r)=>{if(r.kind==="sc")return;if(op!=null)r.alpha=op;if(r.layout==="roll"&&sp){const t=r.paused?r.pausedProgress:(now-r.t0)/Math.max(r.durationMs,1);const wCss=(r.w||100)/Math.max(dpr,0.5);const cssWpx=(canvas.width||1)/Math.max(dpr,0.5);const nd=Math.max(2000,Math.min(20000,((cssWpx+wCss+24)/Math.max(40,sp))*1000));r.durationMs=nd;r.t0=now-t*nd;r.x1=-((r.w||100)+24*dpr);}if((fs||st!=null)&&r.content){const b=bake(r.content,r.kind,fs||config.fontSize,config.stroke);r.u0=b.u0;r.v0=b.v0;r.u1=b.u1;r.v1=b.v1;r.w=b.w;r.h=b.h;}});return;}',
