window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ensureStyles() {
			...
			'.dsh-danmaku-llm-cur{font-size:12px;opacity:.85;max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}',
			'.dsh-danmaku-llm-grid{display:flex;flex-direction:column;gap:10px;margin:2px 0 12px;}',
			'.dsh-danmaku-llm-group{background:var(--dsw-alias-bg-layer-1,rgba(128,128,128,.06));border:1px solid var(--dsw-alias-border-l3,rgba(0,0,0,.1));border-radius:10px;padding:8px 10px;}',
			'.dsh-danmaku-llm-group-none{background:transparent;border:none;padding:0;}',
			'.dsh-danmaku-llm-group-name{font-weight:700;font-size:13px;margin:0 0 6px;opacity:.95;}',
			'.dsh-danmaku-llm-models{display:flex;flex-wrap:wrap;gap:8px;}',
			'.dsh-danmaku-llm-opt{font:inherit;cursor:pointer;border-radius:8px;padding:6px 10px;border:1px solid var(--dsw-alias-border-l4,rgba(0,0,0,.16));background:transparent;color:inherit;text-align:left;line-height:1.25;min-width:0;}',
			'.dsh-danmaku-llm-opt .dsh-danmaku-llm-opt-id{font-size:12px;display:block;}',
			'.dsh-danmaku-llm-opt .dsh-danmaku-llm-opt-name{display:block;font-size:10px;opacity:.6;margin-top:2px;}',
			'.dsh-danmaku-llm-opt:hover{border-color:rgba(251,114,153,.55);}',
			'.dsh-danmaku-llm-opt.selected{border-color:#fb7299;background:rgba(251,114,153,.14);box-shadow:0 0 0 1px #fb7299 inset;}',
				...
				React.createElement('div', {
					...
					weights.map(function (w, i) {
						return React.createElement('div', {
							...
							React.createElement('input', {
								key: 'v', type: 'number', min: 0, max: 100, step: 1,
								value: w.weight || 0,
								style: {
								...
								onChange: function (e) {
									...
									patchAt(i, { weight: Math.max(0, Math.min(100, v)) })
								}
							}),
							React.createElement('button', {
								key: 'x',
		...
		function SettingsPanel(props) {
			...
			function llmModelControl() {
				...
				var children = []
				children.push(React.createElement('div', { key: 'grp-none', className: 'dsh-danmaku-llm-group dsh-danmaku-llm-group-none' }, [
					React.createElement('div', { key: 'm', className: 'dsh-danmaku-llm-models' }, [
						React.createElement('button', {
							key: '__none__', type: 'button',
							className: 'dsh-danmaku-llm-opt' + (cur === '-' ? ' selected' : ''),
							onClick: function () { patch({ llmModel: '-' }) }
						}, React.createElement('span', { className: 'dsh-danmaku-llm-opt-id' }, noneLabel))
					])
				]))
				modelGroups.forEach(function (g) {
					children.push(React.createElement('div', { key: 'grp-' + g.provider, className: 'dsh-danmaku-llm-group' }, [
						React.createElement('div', { key: 'h', className: 'dsh-danmaku-llm-group-name' }, g.provider),
						React.createElement('div', { key: 'm', className: 'dsh-danmaku-llm-models' }, (g.models || []).map(function (m) {
							var id = g.provider + '/' + m.id
							return React.createElement('button', {
								key: m.id, type: 'button',
								className: 'dsh-danmaku-llm-opt' + (cur === id ? ' selected' : ''),
								onClick: function () { patch({ llmModel: id }) }
							}, [
								React.createElement('span', { key: 'id', className: 'dsh-danmaku-llm-opt-id' }, m.id),
								(m.name && m.name !== m.id) ? React.createElement('span', { key: 'nm', className: 'dsh-danmaku-llm-opt-name' }, m.name) : null
							])
						}))
					]))
				})
				return [
					row('llmModel', t('llmModel'), React.createElement('span', { key: 'c', className: 'dsh-danmaku-llm-cur' }, cur === '-' ? noneLabel : cur)),
					React.createElement('div', { key: 'llmGrid', className: 'dsh-danmaku-llm-grid' }, children)
				]
			}
				...
				fetchJson('/api/danmaku/models').then(function (d) {
					...
					groups.forEach(function (g) {
						;(g.models || []).forEach(function (m) {
							list.push({ id: g.provider + '/' + m.id, label: g.provider + ' · ' + (m.name || m.id), provider: g.provider, name: m.name || m.id })
						})
					})
					setModels(list)
				}).catch(function () { setModels([]); setModelGroups([]) })
				...
				React.createElement(CollapseSection, {
					...
					(draft.advancedStyles || []).map(function (s, i) {
						...
						return React.createElement('div', {
							...
							React.createElement('input', {
								key: 'w', type: 'number', min: 0, max: 100, step: 0.5, value: s.weight, style: numStyle,
								onChange: function (e) { setS({ weight: Number(e.target.value) }) }
							}),
							React.createElement('label', { key: 'rl', style: lbl }, t('advRotate')),
							React.createElement('input', {
								key: 'r', type: 'number', min: -30, max: 30, value: s.rotate, style: numStyle,
								onChange: function (e) { setS({ rotate: Number(e.target.value) }) }
							}),
							React.createElement('label', { key: 'sl', style: lbl }, t('advScale')),
							React.createElement('input', {
								key: 'sc', type: 'number', min: 0.8, max: 1.6, step: 0.05, value: s.scale, style: numStyle,
								onChange: function (e) { setS({ scale: Number(e.target.value) }) }
							}),
							...
							React.createElement('input', {
								key: 'o', type: 'number', min: 0.3, max: 1, step: 0.05, value: s.opacity, style: numStyle,
								onChange: function (e) { setS({ opacity: Number(e.target.value) }) }
							}),
							React.createElement('button', {
							...
							exp ? React.createElement('div', {
								...
								React.createElement('input', {
									key: 'du', type: 'number', min: 1000, max: 8000, step: 100, value: s.durationMs || 4000,
									style: numStyle,
									onChange: function (e) { setS({ durationMs: Number(e.target.value) }) }
								}),
		...
		function EmojiEditorModal(props) {
			...
			return React.createElement('div', { className: 'dsh-danmaku-modal', onClick: onClose }, [
						...
						foldersIn.map(function (f) {
							...
							return React.createElement('div', {
								...
								React.createElement('div', {
									...
									React.createElement('input', {
										key: 'wi', type: 'number', min: 0, max: 100, step: 0.5,
										...
										onChange: function (e) { onWeightInput(it, e.target.value) },
										onBlur: function () { onWeightBlur(it) }
									})
								])
							])
						}))
				...
				importDlg ? React.createElement('div', {
					key: 'gitdlg',
					className: 'dsh-danmaku-modal',
					style: { zIndex: 10060 },
					onClick: function () { setImportDlg(false) }
				}, [
					React.createElement('div', {
						key: 'c',
						className: 'dsh-danmaku-modal-card',
				...
				confirmFolder ? React.createElement('div', {
					key: 'fd',
					className: 'dsh-danmaku-modal',
					style: { zIndex: 10060 },
					onClick: function () { setConfirmFolder(null) }
				}, [
					React.createElement('div', {
						key: 'c',
						className: 'dsh-danmaku-modal-card',
