window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function clampConfig(raw) {
			...
			out.llmBurstCount = Math.max(1, Math.min(10, Number(out.llmBurstCount) || DEFAULTS.llmBurstCount))
				...
				items.forEach(function (rec) {
					...
					if (hit && rec.id === hit.id) {
						if (!rec.paused) {
							...
							// t = (uTime - t0 + bias) / dur → bias = p*dur - elapsed
		...
		function createWebgl2WorkerRenderer() {
			...
			function spawnOne(item) {
				...
				setTimeout(function () { hitMap.delete(id) }, durationMs + 200)
			}
			...
			return {
				...
				pauseAll: function () { uiHold = true; post({ type: 'pause-all' }) },
				resumeAll: function () { uiHold = false; post({ type: 'resume-all' }) },
			...
			return React.createElement('div', { className: 'dsh-danmaku-card' }, [
				...
				row('ambient', t('ambient'), React.createElement('input', {
					...
					onChange: function (e) {
						...
						if (Number.isFinite(a) && Number.isFinite(b)) {
							patch({ ambientMinMs: Math.max(400, a), ambientMaxMs: Math.max(a + 200, b) })
						}
					}
				})),
				...
				row('llmModel', t('llmModel'), models && models.length
					...
					: React.createElement('input', {
						...
						placeholder: 'agnes/agnes-2.5-flash',
						onChange: function (e) { patch({ llmModel: e.target.value }) }
					})),
				...
				row('llmBurstCount', t('llmBurstCount'), React.createElement('input', {
					key: 'c', type: 'number', min: 1, max: 10, value: draft.llmBurstCount,
					disabled: !draft.llmEnabled,
		...
		function DanmakuOverlay(props) {
			...
			// Detail modal freezes all scrolling
			React.useEffect(function () {
				var r = rendererRef.current
				if (!r) return
				if (detail) {
					if (r.pauseAll) r.pauseAll()
				} else if (r.resumeAll) {
					r.resumeAll()
				}
			}, [detail])
			...
			React.useEffect(function () {
				...
				if (detail) renderer.pauseAll && renderer.pauseAll()
			...
			React.useEffect(function () {
				...
				function schedule() {
					if (stopped) return
					var span = config.ambientMaxMs - config.ambientMinMs
					var delay = config.ambientMinMs + Math.random() * Math.max(0, span)
					timer = setTimeout(function () {
						...
						if (queueRef.current.length > 0) {
							...
							var cfg = configRef.current
							var items = sampleLocalPresets(cfg.presetPack, 1, cfg.blockedWords)
							if (rendererRef.current && items[0]) {
								rendererRef.current.spawn([{
			...
			}, [config.enabled, config.ambientMinMs, config.ambientMaxMs, config.presetPack, config.blockedWords, config.layoutWeights, sessionId])
			...
			React.useEffect(function () {
				...
				function handleTrigger(kind) {
					if (!inSession()) return
					var cfg = configRef.current
					var count = kind === 'task_done' ? 4 : kind === 'user_message' ? 3 : kind === 'reply_complete' ? 2 : 1
				...
				function poll() {
					...
					fetchJson('/api/danmaku/triggers?since=' + lastSinceRef.current).then(function (data) {
						...
						if (kinds.reply_complete || kinds.task_done || kinds.tool_call || kinds.tool_error || kinds.step || kinds.todo) {
			...
			React.useEffect(function () {
				...
				function scheduleNext() {
					if (stopped) return
					var base = Math.max(5, config.llmIntervalSec || 18) * 1000
					var silence = Date.now() - lastEventAtRef.current
					var delay = base
					if (config.intervalOnlyWhenActive !== false) {
						// Heartbeat: quiet → backoff (12,18,27,40,60s…), active → base interval
						if (silence < 6000) {
							backoffN = 0
							delay = base
						} else {
							var hb = Math.min(60, 12 * Math.pow(1.5, backoffN))
