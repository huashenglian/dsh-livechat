window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function sampleLocalPresets(pack, count, blocked, random) {
			var list = PRESET_PACKS[pack] || PRESET_PACKS.general
			var rnd = random || Math.random
		...
		function createDomRenderer() {
			...
			function spawnOne(item) {
				...
				if (!raw) return
				if (isBlocked(raw, config.blockedWords)) return
				...
				el.dataset.stroke = config.stroke && kind !== 'sc' ? '1' : '0'
				...
				// opacity on inner text so CSS fade keyframes (0→1→0 on outer) multiply correctly
		...
		function SettingsPanel(props) {
			...
			return React.createElement('div', { className: 'dsh-danmaku-card' }, [
				...
				row('presetPack', t('presetPack'), React.createElement('select', {
					key: 'c', value: draft.presetPack,
					onChange: function (e) { patch({ presetPack: e.target.value }) }
				}, [
					React.createElement('option', { key: 'g', value: 'general' }, t('packGeneral')),
					React.createElement('option', { key: 'c', value: 'coding' }, t('packCoding')),
					React.createElement('option', { key: 'l', value: 'casual' }, t('packCasual'))
				])),
