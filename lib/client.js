window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var DEFAULTS = {
			...
			advancedStyles: [
				{ id: 'as_std', name: '常规增强', weight: 3, rotate: 0, scale: 1.05, bold: false, opacity: 1, font: '', durationMs: 4000, mode: 'scroll', xPct: 50, yPct: 40 },
				{ id: 'as_tilt', name: '轻微倾斜', weight: 1, rotate: -6, scale: 1, bold: false, opacity: 0.95, font: '', durationMs: 4000, mode: 'scroll', xPct: 50, yPct: 40 },
				{ id: 'as_big', name: '大号强调', weight: 1, rotate: 0, scale: 1.35, bold: true, opacity: 1, font: '', durationMs: 4000, mode: 'scroll', xPct: 50, yPct: 40 },
			],
			styleTemplates: [
		...
		function clampConfig(raw) {
			...
			out.reverseEnabled = out.reverseEnabled === true
			...
			out.advancedStyles = rawStyles.slice(0, 24).map(function (s, i) {
				...
				var mode = s && (s.mode === 'rain' || s.mode === 'pop') ? s.mode : 'scroll'
				return {
					...
					mode: mode,
					xPct: n(s && s.xPct, 0, 100, 50),
					yPct: n(s && s.yPct, 0, 100, 40)
				}
			})
			...
			if (!out.reverseEnabled) out.layoutWeights.reverse = 0
			else if (!out.layoutWeights.reverse) out.layoutWeights.reverse = 20
		...
		function createDomRenderer() {
			...
			function spawnOne(item) {
				...
				if (adv && adv.mode === 'rain') {
					...
					var pxp = (Number(adv.xPct) || 50) / 100 * lw
					var pyp = (Number(adv.yPct) || 40) / 100 * fullH
					el.style.left = Math.max(0, Math.min(lw - width, pxp - width / 2)) + 'px'
					el.style.top = Math.max(0, Math.min(fullH - lineH(), pyp)) + 'px'
		...
		function SettingsPanel(props) {
			...
			return React.createElement('div', { className: 'dsh-danmaku-card' }, [
				...
				row('reverseEnabled', t('reverseEnabled'), React.createElement('input', {
					key: 'c', type: 'checkbox', checked: draft.reverseEnabled === true,
					onChange: function (e) {
						var on = e.target.checked
						var lw = Object.assign({}, draft.layoutWeights)
						if (on && !Number(lw.reverse)) lw.reverse = 20
						if (!on) lw.reverse = 0
						patch({ reverseEnabled: on, layoutWeights: lw })
					}
				})),
				row('layoutReverse', t('layoutReverse'), React.createElement('input', {
					key: 'c', type: 'number', min: 0, max: 100, value: (draft.layoutWeights.reverse != null ? draft.layoutWeights.reverse : 0),
					disabled: draft.reverseEnabled === false,
					onChange: function (e) { patch({ layoutWeights: Object.assign({}, draft.layoutWeights, { reverse: Number(e.target.value) }) }) }
				})),
					...
					(draft.advancedStyles || []).map(function (s, i) {
						...
						return React.createElement('div', {
							...
							exp ? React.createElement('div', {
								...
								(s.mode === 'pop') ? [
									React.createElement('label', { key: 'xl', style: lbl }, t('advXPct')),
									React.createElement('input', {
										key: 'x', type: 'number', min: 0, max: 100, value: s.xPct != null ? s.xPct : 50,
										style: numStyle,
										onChange: function (e) { setS({ xPct: Number(e.target.value) }) }
									}),
									React.createElement('label', { key: 'yl', style: lbl }, t('advYPct')),
									React.createElement('input', {
										key: 'y', type: 'number', min: 0, max: 100, value: s.yPct != null ? s.yPct : 40,
										style: numStyle,
										onChange: function (e) { setS({ yPct: Number(e.target.value) }) }
									})
								] : null
							]) : null
						])
