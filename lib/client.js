	...
	factory: (require) => {
		...
		function playSvga(url, container, opts) {
			...
			return loadSvgaPlayer().then(function (SVGA) {
				...
				if (!opts.silent) giftToast('error', 'SVGA 素材无法播放', '')
				return handle
			})
		}
		...
		function mountGiftImage(container, url, opts) {
			...
			img.onerror = function () {
				...
				if (!opts.silent) giftToast('error', '素材无法播放', '')
			}
			img.src = url
		...
		function mountGiftSvg(container, url, opts) {
			...
			fetch(url, { credentials: 'same-origin' })
				...
				.catch(function (err) {
					...
					if (!opts.silent) giftToast('error', 'SVG 素材无法播放', '')
				})
			return {
				destroy: function () {
					destroyed = true
		...
		function mountGiftEffectContent(container, asset, opts) {
			...
			if (kind === 'svga') return mountGiftSvga(container, url, opts)
			if (kind === 'image') return mountGiftImage(container, url, opts)
			if (kind === 'svg') return mountGiftSvg(container, url, opts)
		...
		function startGiftEffect(rec, layer) {
			...
			rec.el = el
			try {
				rec.content = mountGiftEffectContent(el, o.asset || null, { loop: loop, durationMs: durationMs, scale: scale })
		...
		function createWebgl2WorkerRenderer() {
			...
			var WORKER_SRC = [
				...
				'const idata=new Float32Array(list.length*INST_FLOATS);const udata=new Float32Array(list.length*8);list.forEach((r,i)=>{const o=i*INST_FLOATS;idata[o]=(r.t0-t0)*0.001;idata[o+1]=r.durationMs*0.001;idata[o+2]=r.paused?r.pausedBiasSec:0;idata[o+3]=r.layout==="roll"?0:1;idata[o+4]=r.x0;idata[o+5]=r.y;idata[o+6]=r.w;idata[o+7]=r.h;idata[o+8]=r.x1;idata[o+9]=r.paused?1:0;idata[o+10]=r.paused?r.pausedProgress:0;idata[o+11]=r.kind==="sc"?1:0;const uo=i*8;udata[uo]=r.u0;udata[uo+1]=r.v0;udata[uo+2]=r.u1;udata[uo+3]=r.v1;udata[uo+4]=r.alpha;udata[uo+5]=r.tr;udata[uo+6]=r.tg;udata[uo+7]=r.tb;});',
					...
					return React.createElement('div', {
						...
						React.createElement('div', { key: 'iw', className: 'dsh-gift-asset-thumb' }, [
							React.createElement('span', {
								key: 'b', className: 'dsh-gift-asset-badge', 'data-kind': it.kind || '',
