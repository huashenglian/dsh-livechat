...
export const GIFT_SENDERS_MAX = 50
/** Max length (chars) of a rendered gift tip. Mirrored by the client.js twin. */
export const GIFT_TIP_MAX = 60
export const GIFT_SENDER_NAME_MAX = 24
...
/**
 * Render the gift tip template (`{user}` / `{gift}` substitution). Pure and
 * never throws: missing/non-string inputs degrade to '', an empty or blank
 * template falls back to the default, unknown tokens are left verbatim, and the
 * result is capped at {@link GIFT_TIP_MAX} chars.
 *
 * Canonical copy — client.js carries an inline twin (a `__ModuleLoader__`
 * factory cannot import Node modules); keep both in lockstep.
 *
 * @param {string} template e.g. '{user} 送出了 {gift}'
 * @param {string} user sender name
 * @param {string} gift gift name
 * @returns {string}
 */
export function renderGiftTip(template, user, gift) {
  const tpl = template == null ? '' : String(template)
  const t = tpl.trim() ? tpl : GIFT_DEFAULTS.giftTemplate
  return t
...
//
// `pickSender` and the batch text helpers are PURE (injectable `rand`, no I/O).
// client.js carries inline twins (a `__ModuleLoader__` factory cannot import
// Node modules); behavior parity is browser-verified by
// tools/verify-gift-config.mjs. Keep the three in lockstep.
// ---------------------------------------------------------------------------

/** Name used when no sender preset is configured. */
export const GIFT_ANONYMOUS_SENDER = '匿名'

function senderWeight(s) {
  const n = Number(s && s.weight)
  if (!Number.isFinite(n)) return 1 // missing / malformed -> default 1
  return n > 0 ? n : 0
}

/**
 * Weighted pick of one sender name. Empty/invalid list -> '匿名'. Weights
 * default to 1; a non-positive total falls back to a uniform pick.
 *
 * @param {Array<{name:string,weight?:number}>} senders
 * @param {() => number} [rand] injectable PRNG in [0,1); defaults to Math.random
 * @returns {string}
 */
export function pickSender(senders, rand) {
  const rnd = typeof rand === 'function' ? rand : Math.random
  const list = (Array.isArray(senders) ? senders : []).filter(
    (s) => s && typeof s === 'object' && String(s.name == null ? '' : s.name).trim() !== ''
  )
  ...
  let total = 0
  for (const s of list) total += senderWeight(s)
  if (total <= 0) {
    // Every weight is 0 -> uniform fallback (still a listed sender).
    return String(list[Math.min(list.length - 1, Math.floor(draw() * list.length))].name)
  }
  let target = draw() * total
  for (const s of list) {
    target -= senderWeight(s)
    if (target < 0) return String(s.name)
  }
  return String(list[list.length - 1].name)
}

/** Split batch text on newlines + ASCII/CJK commas; caller trims. */
function splitSenderLines(text) {
  return String(text == null ? '' : text).split(/[\n\r,，、]+/)
}

/**
 * Parse the sender batch textarea. Line syntax: `名字` or `名字*权重`
 * (weight defaults to 1). Blank lines are skipped; malformed lines are ignored
 * and reported in `invalid` (never throws). Caps at {@link GIFT_SENDERS_MAX}
 * entries; names are truncated to {@link GIFT_SENDER_NAME_MAX} chars.
 *
 * @param {string} text
 * @returns {{senders:Array<{name:string,weight:number}>, invalid:Array<{line:number,raw:string,reason:string}>, overflow:number}}
 */
export function parseSenderBatch(text) {
  const out = []
  const invalid = []
  let overflow = 0
  const lines = splitSenderLines(text)
  for (let i = 0; i < lines.length; i++) {
    ...
    if (star >= 0) {
      const wRaw = trimmed.slice(star + 1).trim()
    ...
    if (out.length >= GIFT_SENDERS_MAX) {
      overflow++
      continue
    }
    out.push({ name: name.slice(0, GIFT_SENDER_NAME_MAX), weight })
  }
  return { senders: out, invalid, overflow }
}

/** Serialize senders back to batch text (round-trips through {@link parseSenderBatch}). */
export function formatSenderBatch(senders) {
  return (Array.isArray(senders) ? senders : [])
    .filter((s) => s && typeof s === 'object' && String(s.name == null ? '' : s.name) !== '')
    .map((s) => {
